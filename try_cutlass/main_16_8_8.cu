/***
 * 这个程序执行正确。
 * 要点1: 保存的tensor2文件并不需要手动转置后保存，制定了LayoutInputB = cutlass::layout::ColumnMajor
 */
...
using ThreadblockShape = cutlass::gemm::GemmShape<16, 8, 8>; // 这里指的是一个block沿K维度滑动，每一次要从AB读取的数据大小
using WarpShape = cutlass::gemm::GemmShape<16, 8, 8>; // 这里指的是一个warp每次从smem读取的数据大小
using InstructionShape = cutlass::gemm::GemmShape<16, 8, 8>; // 这里指一次TensorCore指令读取的数据大小(From smem to reg)
...
constexpr char* tensor_path_1 = "/home/wuyou/cuda_playground/try_cutlass/tensor1_fp16_16_8.bin";
constexpr char* tensor_path_2 = "/home/wuyou/cuda_playground/try_cutlass/tensor2_fp16_8_8.bin";
...
int main() {
    ...
    for(int i = 0; i < tensor_d.size(); ++i) {
        printf("%9.6f%c", tensor_d.host_data()[i], (i + 1) % LEN_K ? ' ' : '\n');
        // if(i > 64)
        //     break;
    }
    printf("\n");
